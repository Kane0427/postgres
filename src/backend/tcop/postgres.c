 ...
 * dgqp code
 *
 * Flag to execute dgqp
 */
static bool open_dynamically_generated_query_plan = false;
 ...
 * dgqp code
 *
 * Execute a "Dynamically Generated Query" protocol message.
 */
static void
exec_dynamically_generated_query(const char *query_string)
{
    CommandDest dest = whereToSendOutput;
    MemoryContext oldcontext;
    List	   *parsetree_list;
			...
			case 'Q':			/* simple query */
				{
					...
					if (am_walsender)
					{
                        ...
                        {
                            if (open_dynamically_generated_query_plan)
                                exec_dynamically_generated_query(query_string);
                            else
